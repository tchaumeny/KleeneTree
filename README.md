...
This project formalizes the Kleene tree in [Lean](https://lean-lang.org/), following (not strictly) a paper of Andrej Bauer (see *references*).
...
shows the non-computable aspect of the (weak) Kőnig's lemma (see e.g. Stillwell's book).

### Main results

##### Kleene tree is computable

Kleene tree is computable, meaning that we can algorithmically decide membership of a `Node`.
...
Kleene tree does not have a computable `Path`: any path (e.g., as given by weak Kőnig's lemma) must be non-computable.
