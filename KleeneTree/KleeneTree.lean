...
import KleeneTree.WeakKonigLemma
...
* `kleene_tree_has_path`: The application of weak Kőnig's lemma to Kleene tree asserts the existence
                          of a path.
...
theorem separation_problem :
  ...
  all_goals
    ...
    simp only [Code.eval_const] at hceval
    let hsepc := hsep c
    rw [hceval] at hsepc
...
theorem kleene_tree_has_path : ∃ p : Path, ∀ n : ℕ, p.get_node n ∈ kleeneTree.nodes :=
  weak_konig_lemma kleene_tree_infinite
